...
export async function backfillCoachSessionSummaries(): Promise<{
  attempted: number;
  succeeded: number;
  failed: number;
}> {
  ...
  const targets = storage.listCoachSessionsNeedingSummary(50);
  if (targets.length === 0) {
    return { attempted: 0, succeeded: 0, failed: 0 };
  }

  console.log(
    `[coach-backfill] starting backfill of ${targets.length} ended sessions without summaries`,
  );
  ...
  console.log(
    `[coach-backfill] done — attempted=${targets.length} succeeded=${succeeded} failed=${failed}`,
  );
