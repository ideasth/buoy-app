...
Last updated: 2026-05-13 02:30 AEST.
...
| 16 | Natural-language scheduling search | `86437c2` + `98254ed` | yes (deployed alongside Stage 17 in the 2026-05-13 02:22 AEST deploy run) | Spec: `STAGE_16_SCHEDULING_SEARCH_SPEC.md`. `POST /api/scheduling/search` accepts `{prompt}` (LLM-parse + search) OR `{parsed}` (search-only refinement — free re-tweaks). Standalone `/#/find-time` page + Find-a-time dialog from Calendar header (one component, two mounts). Per-search source filter chip row: `My Outlook` + `My Buoy events` default ON, family ICS feeds (Marieke / Hilde / Axel) default OFF — chips render only when those feeds exist. 07:00–21:00 Australia/Melbourne window all days; narrowed further by parsed `dateConstraints` + `timePreferences`. 57 new tests (294 total, up from 237 — well above the ≥17 target; ranking matrix exhaustively covered). Net LOC +2510 across 14 files (3 modified, 11 new). LLM model: `sonar-pro` (same as Coach/Calm/Reflect). One real bug caught by tests pre-commit: sign error in `melbDateTime()` Melbourne-local-to-UTC converter. No new tables, no migration, no cron/Outlook/data.db changes. |
| 17 | Three-hostname split: public/private calendars + family | `a4b39c1` → `41d8ae3` (Express 5 hotfix) → `5b224d6` (Settings nav link) | **yes** (2026-05-13 02:22 AEST, bundle md5 `b54d560712360e5d4c9380791d054b98`, pm2 restart #22, health OK on first attempt) | Spec: `STAGE_17_PUBLIC_PRIVATE_CALENDARS_SPEC.md` §K authoritative. Three virtual hosts on a single process. `oliver-availability.thinhalo.com`: sanitised 12-week availability page + Available-only ICS (15-min privacy buffer, 60-min min, 12-week horizon, weekdays 07:00-19:00 + Sat 08:00-13:00 AEST), token-gated 404 without. `buoy-family.thinhalo.com`: week-view family calendar + Add Event + day/week notes + family ICS, Basic auth OR token URL. Apex `buoy.thinhalo.com` untouched — no resubscribe required. New tables: `public_calendar_blocks`, `family_events`, `family_day_notes`, `family_week_notes` (bootstrapped inline). New `app_settings` KV table (12 keys). Hostname routing via `server/hostname-router.ts`. Two new Vite entries. Settings UI on apex at `/#/settings/calendars`, linked from main `/#/settings` page via a "Calendar publishing" card. **Post-ship hotfixes:** first deploy auto-rolled back; pm2 logged `Missing parameter name at index 9: /assets/*` — Express 5 / path-to-regexp 8 rejects bare-`*` wildcards. Fixed `family-routes.ts` + `availability-routes.ts` to use `*splat` named wildcard; added `test/express5-route-patterns.test.ts` as regression guard. 109 new tests + 3 regression tests = 406 total (up from 294). Logo refresh rolled into `a4b39c1`: green life-ring on navy as inline SVG + PNG favicons (32, 180, 192, 512). DNS A records + Caddy vhosts already deployed by operator. **Defaults:** `public_calendar_enabled = private_calendar_enabled = family_calendar_enabled = 0`; user must flip on per calendar at `/#/settings/calendars`. |
...
- **2026-05-13 02:30** — **Stage 17 deployed to VPS** (commit `5b224d6`, bundle md5 `b54d560712360e5d4c9380791d054b98`). First deploy of `a4b39c1` failed health check and auto-rolled back; pm2 logs surfaced `Missing parameter name at index 9: /assets/*` from path-to-regexp 8. Two route patterns (`server/family-routes.ts`, `server/availability-routes.ts`) used bare `*` for static asset catch-all; Express 5 requires named wildcards. Fixed to `*splat` in both files, added `test/express5-route-patterns.test.ts` as regression guard (constructs both routers and asserts bare `*` still throws), committed as `41d8ae3`, redeployed cleanly. Test count 403 → 406. Discovered immediately after that the main Settings page had no link to the new `/#/settings/calendars` page, leaving the three Enabled switches effectively hidden — added a compact "Calendar publishing" link card immediately after the Calendar feed section (commit `5b224d6`), redeployed. Logo refresh rolled into the original `a4b39c1`: user-supplied green-life-ring-on-navy JPEG redrawn as inline SVG for chrome + ImageMagick-generated PNG favicons (32/180/192/512); Login page switched to Logo component; `client/index.html` got apple-touch-icon, theme-color `#0d2a4a`, og:image. All three hostnames serving over HTTPS via Caddy + auto-LE; user verified `https://buoy.thinhalo.com` 200, `https://oliver-availability.thinhalo.com` 404 without `?t=` (correct), `https://buoy-family.thinhalo.com` 404 with feature disabled (correct — "don't advertise the endpoint" pattern). User has not yet enabled any of the three calendar features in `app_settings`. **Pipeline state now:** Shipped = 7–14, 14b, 16, 17 (all deployed); Queued = 15 (Therapy report), 18 (Excel download fix), 19 (Oura).
